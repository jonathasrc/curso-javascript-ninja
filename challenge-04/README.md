...
*/
carro.addPessoa = function(pessoas){
  let vagas = carro.assento - carro.quantidadePessoas;
  ...
  if(pessoas > vagas ){
    let singularPlural = vagas === 1 ? 'pessoa':'pessoas';
    return `só cabe mais ${vagas} ${singularPlural}`;
  } 
...
carro.obterQuantidadePessoa = function() {
